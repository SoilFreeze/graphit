...
# 1. SETUP & PROJECT LOCK
st.set_page_config(page_title="Project Temperature Dashboard", layout="wide")

# REPLACE THIS with the exact name of the project you want to show
TARGET_PROJECT = "North Dam" 

# 2. DATA LOADING (Run this before building the UI)
...
def get_full_dataset():
    # Only pull data for the specific project to save memory/speed
    query = f"SELECT * FROM `sensorpush-export.sensor_data.final_dashboard_data` WHERE project = '{TARGET_PROJECT}' ORDER BY timestamp ASC"
...
# 3. SIDEBAR CONTROLS
st.sidebar.title(f"📁 {TARGET_PROJECT}")
st.sidebar.info("This dashboard is locked to this project's data.")
...
# Standardize values immediately
if is_celsius:
    df_raw['value'] = (df_raw['value'] - 32) * 5/9

df_proj = df_raw.copy() # No selection needed, it's already filtered
