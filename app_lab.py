...
def build_standard_sf_graph(df, title, start_view, end_view, active_refs):
    ...
    Removes all manual datetime math to prevent 'int + datetime' errors.
    """
    try:
        display_df = df.copy()
        
        # 1. FORCE TYPES: Convert everything to Pandas Timestamps immediately
        # This prevents any 'int' or 'NoneType' from entering the math logic
        ...
        # Natural sorting for legend (depth-based)
        ...
        # 4. CUSTOM GRIDLINES (The fix for your error)
        # pd.date_range handles the math internally, avoiding the 'int + datetime' issue
        grid_times = pd.date_range(start=start_ts, end=end_ts, freq='6H')
        
        for ts in grid_times:
            if ts.hour == 0:
                if ts.weekday() == 0:
                    color, width = "DimGray", 2 # Monday Midnight
                else:
                    color, width = "DarkGray", 1.5 # Daily Midnight
            else:
                color, width = "LightGray", 0.8 # 6-hour interval
                
            fig.add_vline(x=ts, line_width=width, line_color=color, layer='below')

        # 5. CURRENT TIME MARKER (The Red Line)
        now_marker = pd.Timestamp.now(tz=pytz.UTC)
        fig.add_vline(x=now_marker, line_width=2, line_color="Red", layer='above',
                      annotation_text="NOW", annotation_position="top")

        # 6. Reference Horizontal Lines
        for val, label in active_refs:
            fig.add_hline(y=val, line_dash="dash", line_color="blue", annotation_text=f"{label} {val}°")
        
        return fig
    except Exception as inner_e:
        # This will show you exactly what part of the function failed
        st.error(f"Graph Logic Error: {inner_e}")
        return go.Figure()
############################
# --- END GRAPH ENGINE --- #
############################
