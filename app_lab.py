...
def build_high_speed_graph(df, title, start_view, end_view, active_refs, unit_mode, unit_label, 
    ...
    - Restored: Global X-Axis Sync (All charts shift to match the project goal window).
    - Updated: Freezing line is a Medium Blue (RoyalBlue) dashed line.
    ...
    # 2. GLOBAL TIMELINE SYNC (The "Shift" Logic)
    final_end_view, final_start_view = end_view, start_view
    
    if f_start_date:
        try:
            # Extract Project ID (e.g., 2527) to find the longest curve in the library
            ...
            if proj_num:
                ...
                if not ref_meta.empty:
                    ...
                    # Lock window: Freezedown Day - 1 to Final Goal Day + 1
    ...
    # Red "NOW" Line (Line only to prevent crashes)
