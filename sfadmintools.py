...
# =============================================================================
# 2. READ-ONLY DATA ENGINE & SANITY FILTERS
# =============================================================================
@st.cache_data(ttl=600)
def get_universal_portal_data(project_id, is_summary_page=False):
    """
    Unified Time-Aware Data Fetcher.
    Joins telemetry to registry by NodeNum AND valid date range (Time-Bound Lock).
    """
...
# =============================================================================
# 3. INTERACTIVE VISUALIZATION COMPILERS & FORMATTERS
# =============================================================================
...
# =============================================================================
# 4. SIDEBAR NAVIGATION SYSTEM & PROJECT SELECTION
# =============================================================================
...
st.sidebar.toggle("Show Theoretical Curves", value=True, key="global_show_ref", help="Superimpose goal curves on Time vs Temp charts.")
st.sidebar.toggle("Show Masked Data", value=False, key="global_show_masked", help="Display data points manually hidden by admins.")
...
tz_mode = st.sidebar.selectbox("Timezone Display", list(tz_lookup.keys()), index=default_tz_index, key="tz_picker")
...
def render_sensor_status(client, selected_project, unit_label, unit_mode, display_tz):
    ...
    # 2. TELEMETRY & COVERAGE QUERY (Uses master_data_view joined with node_registry for active timeline verification)
    query = f"""
        WITH BaseReporting AS (
            ...
            INNER JOIN `{NODE_REGISTRY_TABLE}` n 
              ON m.NodeNum = n.NodeNum
              -- TIME-BOUND LOCK: Exclude sensors that are no longer active or assigned on the project context
              AND m.timestamp >= CAST(n.Start_Date AS TIMESTAMP)
              AND (m.timestamp <= CAST(n.End_Date AS TIMESTAMP) OR n.End_Date IS NULL)
...
if page == "Summary":
    ...
    # Defined inside the final application layer context
